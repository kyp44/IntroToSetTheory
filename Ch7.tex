...
\begin{solution}
    ...
    \proof{
     ...
     \proof{
        ...
        It then follows from Lemma~7.1.6.\ref{lem:hs:onto} that $\abs{\tfun} \leq \abs{\a \times b} = \abs{\a} \cdot \abs{\b}$ since $f$ is onto.
        Hence $\tfun$ is at most $\abs{\a} \cdot \abs{\b}$ as desired. \qedsymbol
     }

     \begin{statement}{Lemma~\ex.\itm{lem:orcarar:seqinf}}
     ...
     \begin{statement}{Corollary~\ex.\itm{cor:orcarar:seqfin}}
        If $A$ is a nonempty finite set then the set $\Seq(A)$ of all finite sequences of elements of $A$ is countable.
     \end{statement}
     \proof{
        First we note that clearly the set $B \cup \w$ is countable (since $B$ is finite and $\w$ is countable) so that $\Seq(A \cup \w)$ is also countable by Lemma~\ex.\ref{lem:orcarar:seqinf}.
        ...
        Thus we have $\al_0 = \abs{\w} \leq \abs{\Seq(a)}$ as well so that $\abs{\Seq(A)} = \al_0$ by the Cantor-Bernstein Theorem. \qedsymbol
     }

     \def\afs{\squares{A}^{< \w}}
     \def\nfs{\squares{\al_\a}^{< \w}}
     \begin{statement}{Corollary~\ex.\itm{cor:orcarar:fss}}
        This is a corollary to Exercise~7.2.3c.
...
\question{\ex}
\begin{solution}
    \begin{statement}{Lemma~\ex.\itm{lem:minus:add}}
        If $\k$ and $\l$ are cardinal numbers and $\k \leq \l < \al_\a$ for some ordinal $\a$, then $\k + \l < \al_\a$.
    \end{statement}
    \proof{
        We have
        \ali{
            \k + \l &= \l + \k & \text{(by the commutativaty of cardinal addition)} \\
        ...
        Hence in either case $\k + \l < \al_\a$ as desired. \qedsymbol
    }
