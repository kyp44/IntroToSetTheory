\begin{lem}\label{lem:ord:lub}
  If $A$ is a set of ordinals then the ordinal $\a$ is the supremum of $A$ if and only if $\a$ is the least upper bound of $A$, i.e. $\a$ is an upper bound of $A$ and $\b$ is not an upper bound of $A$ for any $\b < \a$.
\end{lem}
\qproof{
  \bicproof{
    First suppose that $\a = \sup{A}$.
    ...
    We show that an ordinal $\a$ with the least upper bound property for $A$ is unique, which suffices to show the result since if $\b$ has this property then $\b = \sup{A}$ since $\sup{A}$ does as well (by what was just shown above) and the ordinal having this property is unique.
    ...
    Hence it has to be that $\a = \b$, which shows the uniqueness.
  }
}
